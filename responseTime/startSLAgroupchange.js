...
//globals 
var timeOpen = 0, timePending = 0, statecount = 0, groupcount = 0, commentcount = 0;
var timeUNIX = new Date();

//Set the watch group ID here
var groupid = '91925';
...
var agentFirstrepsonse = function(comments, starts){
            var firstresp = 0;
             userRole(comments).done( function(){ 
               for(ja =1; ja < commentcount; ja++){ 
                   var commentTime = Date.parse(comments['comment' + ja][2])/1000;
                 if(userIDhash[comments['comment' + ja][1]] >= 2 && commentTime > starts && firstresp == 0){     
                    total1 = (Date.parse(comments['comment' + ja][2])/1000) - starts;

                    log_first("1st Resp " + niceTime(total1));
                    jQuery('#ticket_fields_20443383').val(total1);
                    firstresp++;
                }
            }
         }
...
var states = function(previousState, currentState, diff){ 
...
//goes through the group hash and looks for the first change to watch group 
//call the diffTIme and agentFirstrepsonse and passes the time of the watch group is set
var groupSort = function(grouphash){ 
      var timeNow = Date.parse(timeUNIX)/1000; 
      foundgroup = 0;
     for(j = 0; j < groupcount; j++) {  
       if(grouphash['group'+j][4] === groupid && foundgroup == 0) { foundgroup++;
           var groupassigned = Date.parse(grouphash['group'+j][2])/1000
           diffTime(stateHash, groupassigned);
           agentFirstrepsonse(commentHash, groupassigned);
          }
          ...
          $j.getJSON('/tickets/{{ticket.id}}/events.json?filter=audits', function(eventsAudit){
            $j.each(eventsAudit , function(i, item) {
              ...
              $j.each(item.events, function(index, obj) { 
               ...
               statecount++;
              } else if(obj.value_reference === 'group_id'){ 
                groupHash['group' + groupcount] = [obj.value_reference, obj.author_id, obj.created_at,  obj.value_previous, obj.value] ;
                groupcount++;
