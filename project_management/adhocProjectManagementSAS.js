...
<li>
   <label for="multi_group">Groups</label>
   <select id="multi_group" name="multi_group"  multiple="multiple"></select>
 </li>
<li>
...
$j('select#multi_group').multiselect({
            header: "Please select groups",
    ...
    GetGroupData = function(){
        $j.getJSON('/groups.json?page='+ groupPageCount, function(skipgroupobj){
            if ( skipgroupobj.length !== 0) {
                $j.each(skipgroupobj, function(i, item){
                    skipGroup[item.id] = item.name;
                       $j('select#multi_group').append('<option value="'+ item.id+'">'+item.name+'</option>');
                       $j('select#single_group').append('<option value="'+ item.id+'">'+item.name+'</option>');
                       $j('select#multi_group').multiselect("refresh");
...
$j('select#multi_group').change(function(){
          groupListArray = $j('select#multi_group').val();
          //console.log("after sel "+groupListArray);
          }
...
attempt_children = function() {
          ...
          $j.each(groupListArray, function(index, objectValue){
              ...
              rootTicket.ticket.group_id = objectValue;
